...
func init() {
	SetFinalNameParseFunc(func(name sorts.Name) sorts.Code {
		return sorts.Var{Name: name}
	})
}

func init() {
	AddListParseFunc(sorts.TypeCmd, func(parse func(form sorts.Form) sorts.Code, list sorts.List) sorts.Code {
...
func init() {
	AddListParseFunc(sorts.InhabitCmd, func(parse func(form sorts.Form) sorts.Code, list sorts.List) sorts.Code {
...
func init() {
	const (
		LambdaCmd sorts.Name = "=>"
	)
	AddListParseFunc(LambdaCmd, func(parse func(form sorts.Form) sorts.Code, list sorts.List) sorts.Code {
	...
	const ArrowCmd sorts.Name = "->"
	AddListParseFunc(ArrowCmd, func(parse func(form sorts.Form) sorts.Code, list sorts.List) sorts.Code {
...
func init() {
	SetFinalListParseFunc(func(parse func(form sorts.Form) sorts.Code, list sorts.List) sorts.Code {
		err := compileErr(list, []string{
			"cmd",
			"arg1",
			"...",
...
func init() {
	AddListParseFunc(sorts.LetCmd, func(parse func(form form.Form) sorts.Code, list form.List) Code {
		err := compileErr(list, []string{
			string(sorts.LetCmd),
