...
def isName? (form: Form): Option String :=
  match form with
    | .name name => some name
    | _ => none

def isList? (form: Form): Option (List Form) :=
  match form with
    | .list list => some list
    | _ => none
...
instance: Coe String Form where
  coe (s: String) := Form.name s

instance: Coe (List Form) Form where
  coe (l: List Form) := Form.list l
...
structure BlockParser where
  ...
  postProcess: List Form → Option Form
    ...
    | t :: ts =>
      match p.blockParsers.get? t with
        ...
        | some bp => do
          ...
          let form ← bp.postProcess forms.toList
          pure (ts, form)
...
partial def infixLeftToRightProcess (forms: List Form): Option Form := do
  if forms.length ≤ 2 then
    ...
    let opForm ← forms[forms.length-2]?
